...
getDisplayNamesRouter.get(
  '/',
  asyncHandler(async (req: Request<Record<string, unknown>, unknown, unknown, { username?: string }>, res: Response) => {
    ...
    try {
      ...
      return res.json(displayNames.map((row: { display_name: string }) => row.display_name));
    } catch (err) {
      console.error('Error fetching display names:', err);
      return res.status(500).json({ error: 'Failed to fetch display names. Please try again later.' });
    }
  }),
);
